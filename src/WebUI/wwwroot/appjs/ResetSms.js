$(document).ready(function () {
    $('#LanguageMenu').hide();
    var count = parseInt($('#resetsmsTimeOut').val());
...
function SendSms() {
    ...
    $.ajax({
        ...
        url: "Home/ResetPasswordSendSms",
        dataType: "json",
        contentType: 'application/x-www-form-urlencoded; charset=utf-8',
        success: function (result) {
            if (result) {
                window.location.href = window.location.origin + window.location.pathname + "/?sucessmessage=true";
            }
        },
